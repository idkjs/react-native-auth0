...
// type err = string
// type authResult = {"accessToken": string, "idToken": string, "expiresIn": int}
...
// let redirectUri = () => {
//   open Location
//   let portSuffix = port != "" ? j`:$port` : ""
//   j`$protocol//$hostname$portSuffix/callback` //$hostname$portSuffix/callback|j};
// }
...
//       expiresIn: int_of_string(expiresIn),
//     })
//   | (_, _, _) => None
//   }
...
// let jscbToResultcb: (
//   Result.t<'a, string> => unit,
//   Js.null<string>,
//   Js.null<'a>,
// ) => unit = (callback, err, res) =>
//   switch (Js.nullToOption(err), Js.nullToOption(res)) {
//   | (Some(err), _) => callback(Error(err))
//   | (_, Some(res)) => callback(Ok(res))
//   | (None, None) => callback(Error("Both err and res were null in callback"))
//   }

// let userInfo = (
//   accessToken,
//   callback: Result.t<userInfoResult, string> => unit,
